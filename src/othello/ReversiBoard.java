...
public class ReversiBoard {
    ...
    private resultFindMax FindMax(int level, TKind me, TKind opponent) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if ((board[i][j] == TKind.o) && (checkBoard(new Move(i, j), me) != 0)) {
                    if (level != 0) {
    ...
    public boolean findMove(TKind player, int llevel, Move move) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if ((board[i][j] == TKind.o) && (checkBoard(new Move(i, j), player) != 0)) {
                    if (player == TKind.N) {
                        res = FindMax(llevel - 1, TKind.B, player);
                    } else {
                        res = FindMax(llevel - 1, TKind.N, player);
                    }
    ...
    public boolean userCanMove(TKind player) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if ((board[i][j] == TKind.o) && isValid(new Move(i, j), player)) {
                    return true;
                }
            }
        }
